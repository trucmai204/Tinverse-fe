...
interface PaginationResultsProps {
  ...
  className?: string
  alwaysShow?: boolean // Thêm prop để kiểm soát hiển thị
}

export function PaginationResultsFixed({
  currentPage,
  totalPages,
  totalItems,
  itemsPerPage,
  ...
  className = "",
  alwaysShow = true, // Mặc định luôn hiển thị
}: PaginationResultsProps) {
  const effectivePage = Math.max(1, Math.min(currentPage, Math.max(1, totalPages)))
  const startItem = totalItems === 0 ? 0 : (effectivePage - 1) * itemsPerPage + 1
  const endItem = Math.min(effectivePage * itemsPerPage, totalItems)
