...
export function MyArticlesTable() {
  ...
  const [actionType, setActionType] = useState<
    "publish" | "unpublish" | "delete" | "undelete" | null
  >(null);
  ...
  const handleArticleAction = async () => {
    ...
    try {
      ...
      if (currentUser) {
        const articles = await searchAuthorArticles({
          searchTerm,
          page: currentPage,
          itemsPerPage: 10,
          categoryId: selectedCategory,
  ...
  const getActionText = (
    action: "publish" | "unpublish" | "delete" | "undelete" | null
  ) => {
    switch (action) {
      case "delete":
        return "Xóa";
  ...
  const getActionIcon = (
    ...
    switch (action) {
      case "delete":
        return <Trash className="h-4 w-4 mr-2 text-destructive" />;
      case "undelete":
        return <RotateCcw className="h-4 w-4 mr-2 text-blue-500" />;
      case "publish":
        return <Check className="h-4 w-4 mr-2 text-green-500" />;
      case "unpublish":
        return <X className="h-4 w-4 mr-2 text-amber-500" />;
      default:
        return null;
    }
  };
  ...
  const getArticleStatus = (article: any) => {
    if (article.IsDeleted) {
      ...
      return (
        <Badge
          variant="outline"
          className="bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-900 border-amber-200 dark:border-amber-800"
        >
          Chưa xuất bản
        </Badge>
      );
      ...
      return (
        <Badge
          variant="outline"
          className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900 border-green-200 dark:border-green-800"
        >
          Đã xuất bản
        </Badge>
      );
                      ...
                      <TableCell colSpan={6} className="h-24 text-center">
                        <div className="flex flex-col items-center justify-center py-4">
                          <p className="text-muted-foreground">
                            {loading ? "Đang tải..." : "Bạn chưa có Bài báo nào"}
                          </p>
                          {!loading && (
                      ...
                      <TableRow key={article.Id}>
                        <TableCell>
                          <div
                            ...
                            onClick={() =>
                              setPreviewImageUrl(article.ThumbnailUrl || "")
                            }
                          >
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {!article.IsDeleted && (
                                <>
                                  {article.IsPublished ? (
                                    ...
                                    >
                                      <Check className="mr-2 h-4 w-4" /> Xuất
                                      bản
                                    </DropdownMenuItem>
                                  )}
                                  <DropdownMenuItem
                                  ...
                                  >
                                    <Trash className="mr-2 h-4 w-4" /> Xóa bài
                                    viết
                                  </DropdownMenuItem>
                                </>
                              )}
                              {article.IsDeleted && (
                                ...
                                >
                                  <RotateCcw className="mr-2 h-4 w-4" /> Khôi
                                  phục
                                </DropdownMenuItem>
                              )}
                            </DropdownMenuContent>
      ...
      <AlertDialog open={actionDialogOpen} onOpenChange={setActionDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {actionType === "delete" &&
                "Bài báo sẽ bị xóa khỏi hệ thống, nhưng vẫn có thể được khôi phục sau này."}
              {actionType === "undelete" &&
                "Bài báo sẽ được khôi phục và chuyển về trạng thái chưa xuất bản."}
              {actionType === "publish" &&
                "Bài báo sẽ được xuất bản và hiển thị cho người dùng."}
              {actionType === "unpublish" &&
                "Bài báo sẽ bị ẩn khỏi người dùng, nhưng vẫn có thể được xuất bản lại sau này."}
            </AlertDialogDescription>
            {selectedArticle && (
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Ảnh thu nhỏ của Bài báo</DialogDescription>
          </DialogHeader>
